let play_button = document.querySelector('.box3__item1_time2'), music_active = document.getElementById('music'),
    ...
    music_active_img = document.querySelector('.list2 img'),
    ...
    music_active_singer = document.getElementsByClassName('music_name2'),
    ...
    singer_img = document.querySelectorAll('.box_item5 img'),
    ...
    music_list_father = document.querySelector('.box2__item2__time2'), jsmediatags = window.jsmediatags;
...
let music_list = [], singer_list = [];

collect_music(music_list)

singer_img.forEach((item, index) => {
    item.addEventListener('click', () => {
        if (singer_list.includes(item.dataset.id)) {
            item.src = '../static/icons/Rectangle%2062.png'
            let index_singer = singer_list.indexOf(item.dataset.id)
            singer_list.splice(index_singer, 1)
        } else {
            item.src = '../static/icons/square-check-regular%201.png'
            singer_list.push(item.dataset.id)
...
function collect_music() {
    ...
    let music_all_img = document.querySelectorAll('.music .music_img'),
        ...
        music_all_singer = document.getElementsByClassName('music_singer_name'),
...
let category = document.querySelectorAll('.category'), category_id = 0;
...
category.forEach((item, index) => {
    item.addEventListener('click', function () {
        ...
        fetch('/filter_category', {
            method: "POST", body: JSON.stringify({
                category_id: item.dataset.id, singer_list: singer_list
            }), headers: {
                'Content-type': "application/json"
            }
        })
